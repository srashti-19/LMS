...
const AddCourse = () => {
  ...
  const [currenChapterId, setCurrentChapterId] = useState(null)
  ...
  const addLecture = () => {
    setChapters(
      chapters.map((chapter) => {
        if (chapter.chapterId === currenChapterId) {
          const newLecture = {
            ...lectureDetails,
    ...
    <div className='h-screen overflow-scroll flex flex-col items-start justify-between md:pb-0 p-4 pt-8 pb-0'>
      <form onSubmit={handleSubmit} className='flex flex-col gap-4 max-w-md w-full text-gray-500'>
        <div className='flex flex-col gap-1'>
          <p>Course Title</p>
          <input onChange={e => setCourseTitle(e.target.value)} value={courseTitle} type="text" placeholder='Type here...' className='outline-none md:py-2.5 pu-2 px-3 rounded border border-gray-500' required />
        </div>
            ...
            <div key={chapterIndex} className='bg-white border rounded-lg mb-4'>
              <div className='flex justify-between items-center p-4 border-b'>
                <div className='flex items-center'>
                  <img onClick={() => handleChapter('toggle', chapter.chapterId)} src={assets.dropdown_icon} width={14} alt="" className={`mr-2 cursor-pointer tansition-all ${chapter.collapsed && "-rotate-90"}`} />
              ...
              <div className='fixed inset-0 flex items-center justify-center bg-gray-800/50'>
                <div className='bg-white text-gray-700 p-4 rounded relative w-full max-w-80'>
                  ...
                  <div className='mb-2'>
                    ...
                    <input type="number" className='mt-1 block w-full border rounded py-2 px-2'
